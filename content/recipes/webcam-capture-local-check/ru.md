# WebcamCapture: снимок с камеры: локальная проверочная заготовка

Этот фрагмент добавляет только `WebcamCapture`. Обязательные соединяемые входы вынесены наружу, а значения виджетов взяты из закреплённого снимка `/object_info` ComfyUI 0.32.0.
