# TextOverlay: текст поверх изображения: локальная проверочная заготовка

Этот фрагмент добавляет только `TextOverlay`. Обязательные соединяемые входы вынесены наружу, а значения виджетов взяты из закреплённого снимка `/object_info` ComfyUI 0.32.0.
